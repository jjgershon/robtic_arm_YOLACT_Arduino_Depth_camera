...
try:
    ...
    arm.set_servo_values([90, 100, 80, 120])
...
def catch_obj(points3d):
    ...
    if (points3d[2] > 0.1):
        try:
            arm.set_servo_values([base_angle, b, a, 120])
            time.sleep(1)
            arm.set_servo_values([base_angle, b, a, 5])
            time.sleep(2)
            arm.set_servo_values([90, 100, 80, 5])
            time.sleep(1)
            arm.set_servo_values([90, 100, 80, 120])
            catch_yes = 1
        except ValueError:
...
def draw_motion(x_mean,y_mean):
    global draw
    x_mean = -x_mean
    fig, ax = plt.subplots()
    plt.subplots_adjust(left=0.25, bottom=0.25)
    plt.axis([-0.4, 0.2, 0, 0.4])
    ...
    plt.gca().add_patch(plt.Circle([x_mean, y_mean], radius=0.005, color='y'))
    circles = [plt.Circle(p, radius=0.005) for p in pts]
    for c in circles:
        plt.gca().add_patch(c)

    # draw a line for each entry in @LINE_MAPPING
    lines = [plt.Line2D((0, 0), (0, 0), lw=2) for i in range(len(arm_model.LINE_MAPPING))]
    for line in lines:
        plt.gca().add_line(line)

    # update lines from the data in @pts
    def update_lines():
        for i in range(len(arm_model.LINE_MAPPING)):
            src, dst = arm_model.LINE_MAPPING[i]
            p0 = pts[src]
            p1 = pts[dst]
    ...
    # calculate line between init position and point of fruit for animation
    a = (0.03 - y_mean) / (-0.15 - x_mean)
    b = - a * x_mean + y_mean

    def update(i,X,pts,circles):
        I = 50
        if (i <= I):
            x = i * ((x_mean + 0.15) / I) - 0.15
            y = a * x + b
            pts = arm_model.forward(arm_model.inverse_2d([x, y]))
            for i in range(len(arm_model.LINE_MAPPING)):
                src, dst = arm_model.LINE_MAPPING[i]
                p0 = pts[src]
                p1 = pts[dst]
    ...
    animation = FuncAnimation(fig, update, fargs=(X, pts, circles), init_func=update_lines, interval=40)
...
depth_frame = frames_rs.get_depth_frame()
depth_intrin = depth_frame.profile.as_video_stream_profile().intrinsics

color_frame = frames_rs.get_color_frame()

# cfg = pipeline.start() # Start pipeline and get the configuration it found
# profile_ = profile.get_stream(rs.stream.depth) # Fetch stream profile for depth stream
# intr = profile_.as_video_stream_profile().get_intrinsics() # Downcast to video_stream_profile and fetch intrinsics
#
#
# x_g = 0
# y_g = 0
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        inv_alph_masks = masks * (mask_alpha) ## JJ+
        # print(type(inv_alph_masks))

        ## JJ+



        my_mask = masks[0].cpu().numpy()

        sum = 0
        sum_dist = 0

        my_mask = np.squeeze(my_mask, axis = 2)
        # print(my_mask.sum())
        # points = pc.calculate(depth_frame)
        # np_points = np.asanyarray(points.get_texture_coordinates())
        ...
        # points = pc.calculate(depth_frame)
        # np_points = np.asanyarray(points.get_vertices())
        np_mask = np.asarray(my_mask)

        # turn mask into colunm
        vec_mask = my_mask.reshape(307200,1)
        # convert x,y,z points into 3 columns
        verts = vtx.view(np.float32).reshape(-1, 3)
        # merge x,y,z columns with mask column
        table = np.concatenate((verts,vec_mask), axis=1)

        ## debug image
        # np.savetxt("mass.csv", table)
        # np.savetxt("mask.csv", my_mask)
        # np.savetxt("vtx.csv", vtx)
        # np.savetxt("depth.csv", depth_image)


        # np.savetxt("table.csv", table)
        nonZeroMasses = table[np.nonzero(table[:,3])]  # Not really necessary, can just use masses because 0 mass used as weight will work just fine.
        # np.savetxt("nonZeroMasses.csv", nonZeroMasses)
        nonZeroMasses = nonZeroMasses[np.nonzero(nonZeroMasses[:,2])]
        min_dist = np.min(nonZeroMasses[:, 2])
        print(min_dist)


        # heuristics -> removing any pixels further than 20cm from the min distance
        nonZeroMasses = nonZeroMasses[nonZeroMasses[:, 2] < (min_dist+0.2)]
        CM = np.average(nonZeroMasses[:, :3], axis=0, weights=nonZeroMasses[:, 3])

        # list_cm -> center of mass coordinates
        list_cm = [round(x,2) for x in CM.tolist()]
        ...
        # color pixel of CM
        cm_xy = (rs.rs2_project_point_to_pixel(depth_intrin, list_cm))
        global catch_yes
        # enter first reference point
        if (com_counter == 0):
            list_com_x.append(x)
            list_com_y.append(y)
            list_com_z.append(z)
            ...
            if ((abs(x - stat.mean(list_com_x)) < 0.02) & (abs(y - stat.mean(list_com_y)) < 0.02) & (abs(z - stat.mean(list_com_z)) < 0.02)):
                if (com_counter > 5 & catch_yes):
                    # com_counter += 1
                    print("banana! ")
                    x_mean = stat.mean(list_com_x)
                    y_mean = stat.mean(list_com_y)
                    z_mean = stat.mean(list_com_z)

                    print(str(x_mean)+" "+str(y_mean)+" "+str(z_mean))

                    depth_pixel = rs.rs2_project_color_pixel_to_depth_pixel(
                        depth_frame.get_data(), depth_scale,
                        depth_min, depth_max,
                        depth_intrin, color_intrin, depth_to_color_extrin, color_to_depth_extrin, cm_xy)
                    ...
                    '''
                    alpha = (180/math.pi)*math.atan2(y_mean,x_mean)
                    print(alpha)
                    print(math.sqrt(x_mean*x_mean+z_mean*z_mean))

                    ## ANIMATION ++++++++++++++
                    ## ANIMATION ++++++++++++++
                    if draw == 0:
                        length = math.sqrt(x_mean * x_mean + z_mean * z_mean)
                        y_mean = -y_mean
                        print(length, y_mean)
                        if ((length <= 0.25) & (y_mean <= 0.25) & (y_mean >= 0.06)):
                            draw = 1
                            draw_motion(length, y_mean)
        ...
        print("counter = "+str(com_counter))


        cm_xy_int = ()
        for x in cm_xy:
            if not (np.isnan(x) or x == 0):
                cm_xy_int += (round(x),)
        ...
        if len(cm_xy_int) == 2:
            cv2.circle(img_numpy, tuple(cm_xy_int), 3, (0, 0, 255))
        # print(depth_frame.get_distance(cm_xy_int[0],cm_xy_int[1]))
        # depth_colormap =depth_colormap * my_mask #inv_alph_masks.prod(dim=0) #+ masks_color_summand
        images = np.hstack((img_numpy, depth_colormap))
    ...
    #     font_scale = 0.6
    #     font_thickness = 1
    #     cv2.putText(img_numpy, label, text_pt, font_face, font_scale, text_color, font_thickness, cv2.LINE_AA)
    ...
    if num_dets_to_consider == 0:
        images = np.hstack((img_gpu.cpu().numpy(), depth_colormap))
        return images
    ...
    def play_video():
        try:
            ...
            while running:
                ...
                # cv2.setMouseCallback(path, mouse_drawing)
